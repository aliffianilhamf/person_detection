...
while True:
    ...
    #     # Konversi dari format [x_center, y_center, width, height] ke [x1, y1, x2, y2]
    ...
    #     # Pastikan bounding box berada dalam batas citra dan ukurannya valid
    #     if x1 >= 0 and y1 >= 0 and x2 <= frame.shape[1] and y2 <= frame.shape[0] and width > 0 and height > 0:
    #         # Jika valid, tambahkan bbox, confidence, dan oid ke dalam daftar valid
    #         valid_bboxes.append([x_center, y_center, width, height])
    #         valid_confidences.append(detection_scores[i])
    #         if len(oids) > 0:
    #             valid_oids.append(oids[i])

    # # Konversi valid_bboxes kembali ke format numpy array jika diperlukan
    # detection_boxes = np.array(valid_bboxes)
    # detection_scores = np.array(valid_confidences)
    # oids = np.array(valid_oids)   
